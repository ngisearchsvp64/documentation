...
- Compare bytes \texttt{sv.cmpb} (generates the mask \texttt{0xff} because
\texttt{0x17} is the character to match for)
- Compare immediate \texttt{sv.cmpi} (sets bit 1 (GT), and clears bit 2 (EQ),
of CR Field 0)
- Branch conditional (decrements CTR, tests for bit2=0, but doesn't branch)
- Simple-V step \texttt{svstep} (branch was not taken,
